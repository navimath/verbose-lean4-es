...
example (P Q R S : Prop) (h : P ↔ R) (h' : (Q → R) → S) : (Q → P) → S := by
  Como P ↔ R ,y (Q → R) → S concluimos que (Q → P) → S
...
example (n : ℕ) (hn : n > 2) (P : ℕ → Prop) (h : ∀ n ≥ 3, P n) : P n := by
  Como ∀ n ≥ 3, P n ,y n ≥ 3 concluimos que P n
...
example (P Q R : Prop) (h : P → R → Q) (hP : P) (hR : R) : Q := by
  Como P → R → Q basta probar que P ,y R
  exact hP
  exact hR
...
example (P Q : ℝ → Prop) (h : ∀ x > 0, P x → Q x) (h' : P 1) : Q 1 := by
  Como 1 > 0 ,y ∀ x > 0, P x → Q x basta probar que P 1
  guard_hyp_nums 5
  exact h'

example (P Q R S : Prop) (h : P → R → Q → S) (hP : P) (hR : R) (hQ : Q) : S := by
  Como P → R → Q → S basta probar que P, R ,y Q
  exact hP
  exact hR
  exact hQ
...
example (P : ℕ → Prop) (x y : ℕ) (h : x = y) (h' : P x) : P y := by
  Como x = y ,y P x concluimos que P y
...
--   Como (¬ Even n ↔ Odd n) ,y (¬ Even (n^2) ↔ Odd (n^2)) basta probar que Odd n → Odd (n^2)
--   rintro ⟨k, rfl⟩
--   use 2*k*(k+1)
--   ring
...
example (n a b : ℕ) (h : n ≥ max a b) (P : ℕ → Prop) (hP : ∀ n ≥ a, P n) : P n := by
  Como ∀ n ≥ a, P n ,y n ≥ a concluimos que P n
...
⊢ b * (a + b) ≤ b * 3"
  ...
  Como a + b ≤ 3 ,y b ≥ 0 concluimos que b*(a + b) ≤ b*3
...
example (a b : ℝ) (h : a - b ≥ -1) (h' : a - b ≤ 1) : |a - b| ≤ 1 := by
  Como (-1 ≤ a - b ∧ a - b ≤ 1) → |a - b| ≤ 1 basta probar que -1 ≤ a - b ,y a - b ≤ 1
...
example (P : ℝ → Prop) (h : ∀ ε > 0, P ε) : P 1 := by
  Como ∀ ε > 0, P ε ,y 1 > 0 concluimos que P 1
...
example (f : ℕ → ℕ) (A : Set ℕ) (h : ∀ y, ∃ x ∈ A, f x = y) : ℕ → ℕ := by
  Como ∀ y, ∃ x ∈ A, f x = y elegimos g tal que ∀ (y : ℕ), g y ∈ A ,y ∀ (y : ℕ), f (g y) = y
  exact g

example (f : ℕ → ℕ) (A : Set ℕ) (h : ∀ y, ∃ x ∈ A, f x = y) : ℕ → ℕ := by
  Como ∀ y, ∃ x ∈ A, f x = y elegimos g tal que ∀ (y : ℕ), g y + 0 ∈ A ,y ∀ (y : ℕ), f (g y) = y
  exact g
...
example (ε : ℝ) (ε_pos : 1/ε > 0) (N : ℕ) (hN : N ≥ 1 / ε) : N > 0 := by
  Como N ≥ 1/ε ,y 1/ε > 0 concluimos que N > 0
